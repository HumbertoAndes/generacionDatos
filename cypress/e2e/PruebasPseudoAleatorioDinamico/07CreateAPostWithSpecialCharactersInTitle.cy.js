describe('Create a post with special characters in title', ()=> {
  ...
  before(() => {
    cy.request({
      ...
      if (!Array.isArray(dataLogin) || !dataLogin.length || !dataLogin[0].title_character || !dataLogin[0].description_valid) {
  ...
  function random() {
    ...
    it('create a post with special characters in title', ()=>  {  
      ...
      cy.wait(1000)
      cy.get('textarea[placeholder="Post title"]').type(data.title_character)
