describe('Create a post with an extremely long description', ()=> {
  ...
  before(() => {
    cy.request({
      ...
      if (!Array.isArray(dataLogin) || !dataLogin.length || !dataLogin[0].title_valid || !dataLogin[0].description_character) {
  ...
  function random() {
    ...
    it('create a post with an extremely long description', ()=>  {  
      ...
      cy.get('div[class="kg-prose"]').type(data.description_character)  
