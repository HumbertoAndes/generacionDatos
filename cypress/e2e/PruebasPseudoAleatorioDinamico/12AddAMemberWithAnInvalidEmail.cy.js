describe('Add new member with valid data', ()=> {
  ...
  before(() => {
    cy.request({
      ...
      if (!Array.isArray(dataLogin) || !dataLogin.length || !dataLogin[0].name_valid || !dataLogin[0].email_invalid) {
  ...
  it('add new member with valid data', ()=>  {  
    ...
    cy.get('input[id="member-name"]').type(data.name_valid)
    cy.get('input[name="email"]').type(data.email_invalid)        
  })
})